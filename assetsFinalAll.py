...
class Backtest:
    ...
    def __init__(self, strategy, weighting, trading_freq, roll_data, all_assets):
        ...
        with open(os.path.join("results_FINAL_all", "backtest_summary_FINAL_all.txt"), "a") as file:  # 'a' mode appends to the file without overwriting
    ...
    def save_results(self):
        self.__all_assets.get_merged_df().to_csv(os.path.join("results_FINAL_all", f"{self.__all_assets.get_category()}_{self.__strategy}_{self.__trading_freq}_{self.__weighting}_{self.__roll_data}_data.csv"))
        self.__all_returns.to_csv(os.path.join("results_FINAL_all", f"{self.__all_assets.get_category()}_{self.__strategy}_{self.__trading_freq}_{self.__weighting}_{self.__roll_data}_returns.csv"))
        self.__signals.to_csv(os.path.join("results_FINAL_all", f"{self.__all_assets.get_category()}_{self.__strategy}_{self.__trading_freq}_{self.__weighting}_{self.__roll_data}_signals.csv"))
        self.__all_trades.to_csv(os.path.join("results_FINAL_all", "trades", f"{self.__all_assets.get_category()}_{self.__strategy}_{self.__trading_freq}_{self.__weighting}_{self.__roll_data}_trades.csv"))



assets_to_use = all_df

        ...
        if category == "all":
            ...
            for category, assets in all_assets.items():
                for asset in assets:
                    ...
                    new_df.loc[:first_day, f"{asset.get_name()}_start"] = "excluded"
                    df = df.join(new_df, how="left")
        elif category == "equity_bonds_partial_commodity":
            all_assets_new = {k:v for k,v in all_assets.items() if k in ("equity", "bonds")}
            all_assets_new["commodity"] = [x for x in all_assets["commodity"] if x.get_name() in ("CL1_Comdty", "HO1_Comdty", "GC1_Comdty")]
...
equity_bonds_partial_commodity_df = All_assets("equity_bonds_partial_commodity", all_assets, "1997-08-15")
#########################################################################################################################
...
class Backtest:
    ...
    def __init__(self, strategy, weighting, trading_freq, roll_data, all_assets):
        ...
        with open(os.path.join("results_FINAL_equityBondsPartialCommodity", "backtest_summary_FINAL_equityBondsPartialCommodity-E.txt"), "a") as file:  # 'a' mode appends to the file without overwriting
    ...
    def save_results(self):
        self.__all_assets.get_merged_df().to_csv(os.path.join("results_FINAL_equityBondsPartialCommodity", f"{self.__all_assets.get_category()}_{self.__strategy}_{self.__trading_freq}_{self.__weighting}_{self.__roll_data}_data.csv"))
        self.__all_returns.to_csv(os.path.join("results_FINAL_equityBondsPartialCommodity", f"{self.__all_assets.get_category()}_{self.__strategy}_{self.__trading_freq}_{self.__weighting}_{self.__roll_data}_returns.csv"))
        self.__signals.to_csv(os.path.join("results_FINAL_equityBondsPartialCommodity", f"{self.__all_assets.get_category()}_{self.__strategy}_{self.__trading_freq}_{self.__weighting}_{self.__roll_data}_signals.csv"))
        self.__all_trades.to_csv(os.path.join("results_FINAL_equityBondsPartialCommodity", "trades", f"{self.__all_assets.get_category()}_{self.__strategy}_{self.__trading_freq}_{self.__weighting}_{self.__roll_data}_trades.csv"))



assets_to_use = equity_bonds_partial_commodity_df
